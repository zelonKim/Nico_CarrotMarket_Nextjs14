...
const SUPABASE_PUBLIC_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9xbG9waG9ybHdodmNkb2Fkb2ViIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTkxMTY4NjAsImV4cCI6MjAzNDY5Mjg2MH0.aWUIWfvsaQwmanF_kYxftKmBlgaMjIzgSgbZviVLAh0";
...
export default function ChatMessagesList({
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    channel.current?.send({
      // 채널.send({ type:"타입명", event:"이벤트명", payload:{페이로드} }): 해당 채널로 인수들을 보냄.
      type: "broadcast",
      event: "message",
      payload: {
        id: Date.now(),
  ...
  useEffect(() => {
    ...
    channel.current
      .on("broadcast", { event: "message" }, (args) => {
        // 채널.on("타입명", {event: "이벤트명"}, 콜백함수): 채널을 설정함.  // 콜백함수의 매개변수에는 채널.send()메서드로 보낸 인수들이 담김.
